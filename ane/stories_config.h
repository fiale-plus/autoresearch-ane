...
typedef struct {
    Kern *sdpaFwd;     // QKV matmul + SDPA (no Wo) [legacy, kept for now]
    Kern *woFwd;       // attn_out @ Wo^T → o_out  [legacy, kept for now]
    ...
    Kern *sdpaBwd1;    // Q,K,V,da → dV,probs,dp [legacy, kept for now]
    Kern *sdpaBwd2;    // probs,dp,Q,K → dQ,dK   [legacy, kept for now]
    Kern *qBwd;        // dq @ Wq → dx_q          [legacy, kept for now]
    Kern *kvBwd;       // dk@Wk + dv@Wv → dx_kv   [legacy, kept for now]
    Kern *sdpaWoFwd;    // fused SDPA + Wo forward
    Kern *sdpaBwdFused; // fused SDPA backward (replaces sdpaBwd1+sdpaBwd2)
    Kern *qkvBwd;       // fused Q+KV backward (replaces qBwd+kvBwd)
} DynLayerKernels;
...
typedef struct {
    ...
    IOSurfaceRef sdpaWoFwd_in, qkvBwd_in;
} PerLayerSurfaces;
...
typedef struct {
    ...
    void *sdpaWoFwd, *sdpaBwdFused, *qkvBwd;
} PerLayerRequests;
